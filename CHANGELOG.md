...
- Добавлено свойство `IEsiaEnvironment.EsiaCertificates`, содержащее набор сертификатов среды ЕСИА
  (прошлые, текущий и, если он уже известен, будущий).
  Все они передаются в `TokenValidationParameters.IssuerSigningKeys`, поэтому смена сертификата на стороне ЕСИА
  больше не приводит к отказу в обслуживании.
  Реализация по умолчанию возвращает единственный `EsiaCertificate`, поэтому собственные реализации
  `IEsiaEnvironment` продолжат работать без изменений.
- В набор сертификатов промышленной среды добавлен предыдущий сертификат (выпущен 29.04.2025),
  чтобы переход на текущий сертификат (выпущен 23.04.2026) проходил без простоя.

### Устарело

- Свойство `IEsiaEnvironment.EsiaCertificate` помечено как устаревшее и будет удалено в следующей мажорной версии.
  Используйте `IEsiaEnvironment.EsiaCertificates`.

## Версия 2.2.2

- Обновлён сертификат промышленного контура ЕСИА в `ProductionEsiaEnvironment`.

## Версия 2.2.1

- Исправлена зависимость System.Security.Cryptography.Pkcs для .NET 10.
